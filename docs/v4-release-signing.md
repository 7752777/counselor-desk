...
没有签名的 `.msi` 或网页离线包仍然可以用于本机测试，但 Windows SmartScreen 可能显示“未知发布者”并要求用户额外确认。只有签名验证结果为 `Valid` 的构建，才可以作为正式公开安装包发布。
...
当前仓库没有内置正式证书，签名门禁会有意失败。这是安全设计，不是功能故障：开发和验收可以使用未签名体验包，但它必须明确标注为体验发布；正式公开发行前再由发布者配置自己的证书和密钥保管流程。
