...
- **v3.5 数据闭环**：18 个业务模块补齐 CSV 导入 + 模板下载（导出的文件可原样导回）；JSON 备份恢复现一并还原个人设置（辅导员姓名 / 学院 / 隐私锁）；新增 3 套本地测试（回归 / 导入导出闭环 / 全模块增删改）。
- **v3.6 视觉与交互升级**：重构设计令牌系统（模块化字号阶梯 / 深邃品牌蓝 / 分层阴影 / 8pt 间距 / 统一动效曲线）；新增完整深色模式（顶栏一键切换、localStorage 持久化、首次进入跟随系统偏好）；按钮悬浮抬升、输入框聚焦环、卡片双描边、表格数字等宽与行 hover、空状态插画；全局 :focus-visible 与 prefers-reduced-motion 无障碍支持。
...
```
counselor-desk/
├── index.html              # 全部功能（零外部依赖，单文件 ~5600 行）
├── tests/                  # 本地测试（需 jsdom，三个都应 PASS）
│   ├── regression.js       # 回归测试：22 视图渲染 + Phase A/B/C 关键特性
│   ├── import-loop.js       # 导入/导出闭环：18 模块 CSV 往返 + JSON 备份恢复
│   ├── crud-smoke.js       # 全模块增删改冒烟：新建 / 编辑 / 删除 / 必填校验
│   └── student-import.js    # v3.8 学生大表导入：列名匹配 / 缺值容错 / 合并契约
├── README.md               # 本文件
├── docs/
│   ├── 使用指南.md          # 各模块怎么用（v3.8.0 更新：xls/xlsx 学生大表导入）
